...
export const calculateYearlyEarningsForAsset = ({
  ...
  const yearlyEarningsCents = supplyYearlyEarningsCents.plus(
    borrowYearlyEarningsCents
  );
  ...
  const supplyYearlyNwlDistributionEarningsCents =
    supplyYearlyEarningsCents.multipliedBy(asset.nwlSupplyApr.dividedBy(100));

  const borrowYearlyNwlDistributionEarningsCents =
    borrowYearlyEarningsCents.multipliedBy(asset.nwlBorrowApr.dividedBy(100));
...
export const calculateYearlyEarningsForAssets = ({
  ...
  if (yearlyEarningsCents) {
    ...
    yearlyEarningsCents = yearlyEarningsCents.plus(
      yearlyNwlDistributionInterestsCents
    );
  }
