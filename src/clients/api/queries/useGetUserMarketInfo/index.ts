...
const useGetUserMarketInfo = ({
  ...
  const { data: userMintedNaiData, isLoading: isGetUserMintedNaiLoading } =
    useGetMintedNai(
      {
  ...
  const nTokenBalances = useMemo(
    () =>
      indexBy(
        (item: IGetNTokenBalancesAllOutput["balances"][number]) =>
          item.nToken.toLowerCase(),
  ...
  const data = useMemo(() => {
    const {
      ...
      (acc, market) => {
        ...
        const asset = {
          ...
          treasuryTotalBorrowsCents: new BigNumber(
            market?.totalBorrowsUsd || 0
          ).times(100),
          treasuryTotalSupplyCents: new BigNumber(
        ...
        // user totals
        const borrowBalanceCents = asset.borrowBalance
          .times(asset.tokenPrice)
          .times(100);
        const supplyBalanceCents = asset.supplyBalance
          .times(asset.tokenPrice)
          .times(100);
        acc.userTotalBorrowBalanceCents =
          acc.userTotalBorrowBalanceCents.plus(borrowBalanceCents);
        acc.userTotalSupplyBalanceCents =
          acc.userTotalSupplyBalanceCents.plus(supplyBalanceCents);
    ...
    const userTotalBorrowBalanceWithUserMintedNai =
