...
const useGetNaiVault = ({
  accountAddress,
}: {
  accountAddress?: string;
}): UseGetNaiVaultOutput => {
  const { data: totalNaiStakedData, isLoading: isGetTotalNaiStakedWeiLoading } =
    useGetBalanceOf(
      {
  ...
  const {
    data: userPendingNaiRewardData,
    isLoading: isGetUserPendingNaiRewardWeiLoading,
  } = useGetNaiVaultPendingNwl(
    {
      accountAddress: accountAddress || "",
    },
  ...
  const {
    data: naiVaultDailyRateData,
    isLoading: isGetVaiVaultDailyRateWeiLoading,
  } = useGetNarwhalNaiVaultDailyRate();

  const { data: nwlPriceDollars, isLoading: isGetNwlPriceLoading } =
    useGetNwlPrice();
