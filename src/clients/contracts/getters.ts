...
export const getTokenContract = (token: Token, web3: Web3) => {
  if (token.address === TOKENS.nwl.address) {
    return getContract<TokenContract<"nwl">>(
      NwlTokenAbi as AbiItem[],
      token.address,
      web3
    );
  }
...
export const getNTokenContract = <T extends string>(
  tokenId: T,
  web3: Web3
): NTokenContract<T> => {
...
export const getFaucetContract = (web3: Web3) =>
  getContract(
    FaucetAbi as AbiItem[],
    getContractAddress("faucet"),
    web3
